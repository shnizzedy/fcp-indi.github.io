...
.. note::
   ...
   C-PAC automatically creates a JSON-like file called ``callback.log`` (via the function :ref:`log_nodes_cb <#CPAC.utils.montioring.log_nodes_cb>`) when running. This file includes each Node's estimated memory and memory observed at runtime. The Nipype utility function :ref:`log_to_dict <#nipype.utils.draw_gantt_chart.log_to_dict>` reads a log file generated by ``log_nodes_cb`` to a Python dictionary.
